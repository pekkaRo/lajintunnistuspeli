...
import { Leaf, Fish, Award, ArrowRight, CheckCircle, XCircle, RefreshCw } from 'lucide-react';
    ...
    items: [
      {
        ...
        hints: ['Kasvaa havumetsissä', 'Tummansinisiä marjoja', 'Värjää kielen siniseksi', 'Marjat ovat pyöreitä'],
        funFact: 'Mustikan syöminen parantaa näköä hämärässä!'
      },
      {
        name: 'Puolukka',
        wikiTitle: 'Vaccinium vitis-idaea',
        hints: ['Kasvaa kangasmetsissä', 'Kirkkaan punaisia marjoja', 'Marjat ovat hieman happamia', 'Talvivihreä kasvi'],
      ...
      },
      {
        ...
        hints: ['Kasvaa rannoilla', 'Kirkkaankeltaisia marjoja', 'Okaiset oksat', 'Marjat tiheissä rypäleissä'],
        funFact: 'Tyrni sisältää paljon C-vitamiinia!'
      },
      {
        name: 'Mesimarja',
        wikiTitle: 'Rubus arcticus',
        hints: ['Kasvaa kosteissa metsissä', 'Keltaisia tai punaisia marjoja', 'Muistuttaa vadelmaa', 'Makeanhapan maku'],
...
const LajintunnistusPeli = () => {
  ...
  const currentCategoryData = currentCategory
    ? (categories[currentCategory as keyof typeof categories] as Category)
    : null;
  const currentSpeciesKey = currentCategory && currentSpecies
    ? getSpeciesKey(currentCategory, currentSpecies.name)
    : null;
  useEffect(() => {
  ...
  useEffect(() => {
    if (!currentCategory || !currentSpecies || !currentSpeciesKey) return;

    const cacheKey = currentSpeciesKey;
    const existing = imageCache[cacheKey];
    if (existing && (existing.status === 'loading' || existing.status === 'loaded')) {
      return;
    }

    const fetchImageData = async () => {
      setImageCache((prev) => ({ ...prev, [cacheKey]: { status: 'loading' } }));

      try {
        const wikiTitle = currentSpecies.wikiTitle;
        const baseResponse = await fetch(
          `https://commons.wikimedia.org/w/api.php?action=query&titles=${encodeURIComponent(
            wikiTitle
          )}&prop=pageimages|pageterms&piprop=original&pithumbsize=800&format=json&origin=*`
        );
        const baseJson = await baseResponse.json();
        const pages = baseJson?.query?.pages || {};
        const page = Object.values(pages)[0] as any;

        if (!page || (!page.thumbnail && !page.original) || !page.pageimage) {
          throw new Error('Kuvaa ei löytynyt');
        }

        const thumb = page.thumbnail?.source || page.original?.source || '';
        const fileTitle = page.pageimage as string;
        let src = page.original?.source || thumb;
    ...
    fetchImageData();
  }, [currentCategory, currentSpecies, currentSpeciesKey, imageCache]);

  const selectRandomSpecies = () => {
    if (!currentCategory) return;

    const categoryKey = currentCategory as CategoryKey;
    const categoryData = categories[categoryKey as keyof typeof categories] as Category;
    const availableSpecies = categoryData.items.filter(
      (item: Species) =>
        !usedSpecies.some((used: { name: string; category: CategoryKey }) => used.name === item.name && used.category === categoryKey)
    );

    if (availableSpecies.length === 0) {
      setUsedSpecies([]);
      const randomIndex = Math.floor(Math.random() * categoryData.items.length);
      setCurrentSpecies(categoryData.items[randomIndex]);
    } else {
      const randomIndex = Math.floor(Math.random() * availableSpecies.length);
      setCurrentSpecies(availableSpecies[randomIndex]);
    }
  ...
  const generateWrongAnswers = (correctAnswer: string): Species[] => {
    if (!currentCategory) return [];

    const categoryKey = currentCategory as CategoryKey;
    const currentItems = categories[categoryKey as keyof typeof categories].items as Species[];
    const wrongAnswers = currentItems.filter((item: Species) => item.name !== correctAnswer);
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8 md:mb-12">
            ...
            <p className="text-lg md:text-xl text-gray-700">
              Valitse kategoria ja testaa tietosi!
            </p>
          </div>
  ...
  const answerOptions: Species[] = [currentSpecies, ...generateWrongAnswers(currentSpecies.name)];
  const answers = [...answerOptions].sort(() => Math.random() - 0.5);

  const imageStatus = currentImageData?.status;
  const imageDetails = currentImageData?.data;
            ...
            <div className="lg:w-1/2">
              <div className="bg-white border-2 border-green-100 rounded-xl shadow-inner h-full flex flex-col items-center justify-center p-4">
                {imageStatus === 'loaded' && imageSrc ? (
                  ...
                  <div className="w-full h-56 md:h-64 rounded-xl bg-gradient-to-r from-green-100 via-blue-100 to-green-100 animate-pulse"></div>
                ) : imageStatus === 'error' ? (
                  <div className="w-full h-56 md:h-64 flex flex-col items-center justify-center bg-green-50 rounded-xl text-center text-sm md:text-base text-green-700 px-4">
                    <span className="text-3xl mb-2" role="img" aria-label="metsä">🌲</span>
                    <p>Kuva puuttuu, mutta voit kuvitella mielessäsi miten {currentSpecies.name.toLowerCase()} näyttää luonnossa.</p>
                  </div>
                ) : (
            ...
            <div className="lg:w-1/2">
              <div className="bg-green-50 rounded-xl p-4 md:p-6 h-full">
                ...
                <ul className="space-y-2">
                  {currentSpecies.hints.map((hint, index) => (
              ...
              return (
                ...
                >
                  ...
                  {showFeedback && isCorrect && (
                    <CheckCircle className="inline ml-2 w-5 h-5 md:w-6 md:h-6" />
                  )}
                  {showFeedback && isSelected && !isCorrect && (
                    <XCircle className="inline ml-2 w-5 h-5 md:h-6" />
                  )}
                </button>
              );
            })}
              ...
              {selectedAnswer === currentSpecies.name ? (
                ...
                <div>
                  ...
                  <p className="text-sm md:text-base text-gray-700 mb-3">
                    Oikea vastaus oli <strong className="text-green-700">{currentSpecies.name}</strong>.
                  </p>
...
export default LajintunnistusPeli;